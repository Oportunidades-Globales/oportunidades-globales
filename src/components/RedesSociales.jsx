...
const RedesSociales = ({ className = '', showLabels = false, size = 'md' }) => {
  ...
  const redes = [
    {
      ...
      disponible: redesSociales.facebook !== 'queda pendiente'
    },
         {
       nombre: 'Instagram',
    ...
    {
      nombre: 'Gmail',
      url: `mailto:${redesSociales.gmail}`,
      icono: (
        <svg className={getIconSize()} fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className={`flex items-center space-x-4 ${className}`}>
      {redes.map((red) => (
        <div key={red.nombre} className="flex flex-col items-center">
          {red.disponible ? (
            <a
              href={red.url}
            ...
            <div className={`text-gray-400 cursor-not-allowed ${getIconSize()}`}>
              {red.icono}
            </div>
          )}
          {showLabels && (
            <span className={`text-xs text-gray-500 mt-1 ${getTextSize()}`}>
              {red.nombre}
            </span>
          )}
        </div>
